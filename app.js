...
class Triangle extends Shape {
    constructor(shapeName, top, left, width, height, area, perimeter) {
        ...
        this.div.css({
            ...
            "width": `0px`,
            "height": `0px`,
            "border-top": `${this.height}px solid yellow`,
...
triBtn.click(() => {
    this.triHeightInput = $(`#triHeightInput`).val();

    let width = this.triHeightInput;
    let height = this.triHeightInput;
    let top = Math.floor(Math.random() * (600 - height));
    let left = Math.floor(Math.random() * (600 - width));
    let area = (height * width * 0.5);
    let perimeter = ((height * 2) + (Math.sqrt(2) * height));

    if (triHeightInput > 600) {
        alert('The maximum input is 600. Try again!')
        return;
    }

    let newTri = new Triangle('triangle', top, left, width, height, area, perimeter);
